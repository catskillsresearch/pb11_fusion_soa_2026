...
One row per major effort in Table 2. Detail and scoring live in Table 8 (§8) and §6. Device cutaways and concept figures for the major paths are archived under `research/figures/` (see `research/figures/CREDITS.md` for provenance).
...
*   **Executive path (≈20 minutes):** §1 (this section, including the §1.3.1 maps) → skim §5 (are other fuels as clean?) → Table 8 in §8 → **§12.1 ranked plant-odds table** → §11 patent/legal footprint if diligence matters. Skip the equations. Optional: one company page in §6 that matches a pitch you just heard.
...
Adapted for **electricity from $p\text{-}^{11}\text{B}$**, the gates used throughout §§6–8 and Table 8 are shown in Table 3.
...
| **Blue Laser Fusion** / **Anubal Fusion** | High-rep laser–target ICF options in Table 8—same confinement family as Marvel/HB11. |
...
Several key players are actively developing $p\text{-}^{11}\text{B}$ systems geared toward $Q > 1$ commercial electricity generation. Subsections below answer the Table 3 gates in fixed order (**F**, **K**, **R**, **A**, **L**, **C**, **M**, **T**; **B** only if relevant).
...
### Table 7: Princeton / PPPL spinout comparative prognosis (mid-2026)
...
Table 8 scores principal $p\text{-}^{11}\text{B}$ (and closely related) paths on the Table 3 zeroth-order gates. Sister advanced-fuel rows (Helion, PFS) and compact MEC/IEC rows (Avalanche Orbitron; JIHT Kurilenkov NVD) are included for contrast after §4 / §5. Legend: **●** = publicly articulated strategy with supporting hardware, peer-reviewed analysis, or formal design review; **◐** = partial / roadmap-level / component-only; **○** = weak, absent, or not yet public; **—** = not applicable. Scores are editorial judgments for comparison, not certifications. §12.1 folds these cells into a weighted **Plant Odds Score (POS★)**—with a $\kappa$ haircut for staged-fuel and imputed theory plants—and sorts each path on its own rank with indicative timelines.

### Table 8: Zeroth-order scorecard for key projects (State of the Art, 2026)
...
**Plain-language lead-in.** Who designs on computers how fast (**S**), versus who builds and shoots hardware how often (**H**). Table 9 is a shopping list for tools and how to obtain them—open stacks vs closed software-as-a-service (SaaS).
...
### Table 9: Selected fusion design / digital-twin software (access snapshot, mid-2026)
...
### Table 10a: Corporate / brand legal footprint (non-patent)
...
### Table 10b: Representative granted patents and published applications (expanded sweep)
...
### Table 10c: What the citation graph shows (cross-firm prior art)
...
**Metric (Plant Odds Score, POS★).** Map Table 8 cells as $\bullet=2$, $\circledcirc=1$, $\circ=0$; treat $—$ as $0$ for plant purposes. Base score
...
### Table 11: Ranked $p\text{-}^{11}\text{B}$ plant odds (editorial, mid-2026)
...
7. **Marvel Fusion** — Strong nano-target **K**; plant **L** weaker than HB11’s in Table 8; CSU still standing up.
...
¶ LPPFusion POS from §6.3 narrative (not a full Table 8 row); approximate.  
