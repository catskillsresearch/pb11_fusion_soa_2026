...
| :--- | :--- | :--- |
| **Time** | Does the machine run steadily, or in shots/pulses? | Continuous / quasi-steady magnetic plants; pulsed lasers; pulsed pinches / FRC collisions; nanosecond vacuum discharges |
| **Confinement family** | What holds the fuel together long enough to fuse? | Closed magnetic (tokamak, spherical torus, stellarator); compact magnetic (FRC); open magnetic / rotating mirror (CHARM); laser / inertial / beam-target; magneto-inertial (DPF); electrostatic / magneto-electrostatic (Orbitron, NVD–IEC) |
...
| :--- | :--- | :--- | :--- | :--- |
| **ENN (China)** | Continuous | Spherical torus | $p\text{-}^{11}\text{B}$ | Largest national ST + nuclear-data program; demo roadmap ~2030s |
| **TAE Technologies** | Quasi-steady | Beam-driven FRC | $p\text{-}^{11}\text{B}$ | Deepest private FRC capital; LHD collaboration on alphas |
| **Pale Blue / CHARM (Princeton)** | Continuous open-field concept | Rotating multi-chamber mirror | $p\text{-}^{11}\text{B}$ | Strong theory + young IP; still incorporating; software-first |
| **HB11 Energy** | Pulsed | Laser block ignition | $p\text{-}^{11}\text{B}$ | Commercial laser path; avalanche / DEC claims debated |
| **Marvel / Blue Laser / Anubal** | Pulsed | Laser / nano-ICF | $p\text{-}^{11}\text{B}$ (options) | European / CA laser startups; facilities and patents growing |
| **LPPFusion** | Pulsed | Dense plasma focus | Advanced / $p\text{-}^{11}\text{B}$ | Long-running DPF company |
...
| **JIHT / Kurilenkov** | Pulsed ns | NVD–IEC virtual cathode | $p\text{-}^{11}\text{B}$ | Russian lab alphas + KARAT PIC scaling; same electrostatic neighborhood as Avalanche |
| **Helion / PFS** | Pulsed / small FRC | FRC / PFRC | D–$^3$He | Serious capital (Helion) or microreactor line (PFS); **cleaner than D–T, not $p\text{-}^{11}\text{B}$-clean** |
| **Thea Energy** | Continuous | Planar-coil stellarator | D–T | Princeton spinout; DOE Helios path—sister company, different fuel |
| **LHD / NIFS, PROBONO, CN HEDP** | Facility / consortium | Helical / laser targets | $p\text{-}^{11}\text{B}$ experiments | Science infrastructure, not one commercial plant |
...
| **A — Ash & impurities** | How is $^4\mathrm{He}$ removed on a timescale $\ll\tau_E$? Wall/$Z_{eff}$ poisoning controlled? (§3.6, §9) |
...
| **C — Confinement class** | MCF (ST, FRC, helical, open-field mirror), ICF / HEDP, magneto-inertial / DPF, **MEC / IEC** (Orbitron-class), or hybrid? (§4) |
| **M — Materials & energy capture** | First wall, electrodes/grids, switches, divertor; thermal cycle vs direct conversion (ICC, electrostatic grids, photoelectric). |
| **B — Breeding** | N/A for pure $p\text{-}^{11}\text{B}$; relevant only when a sister D–T / D–$^3$He path is discussed (e.g., Thea, PFRC). |
| **T — Technology-to-market** | Device generation on the floor, federal milestones, capital, IP, and brand maturity (§5.7). |
| **S — In-silico / digital-twin iteration** | Can the team close design loops in software (0D→CAD→neutronics→cost) faster than hardware rebuilds? Open-source vs closed; who can obtain and run the tools? (§8) |
...
### 4.1 Alternative Aneutronic Fuels and Reactions
...
**Candidate primary channels** (no neutrons on the listed branch) include [77]:
...
**Who is pursuing the alternatives (mid-2026)?** Commercial activity outside $p\text{-}^{11}\text{B}$ clusters almost entirely on **D–${}^3\mathrm{He}$ as a neutron-*reduced* stopgap**, not on fuels that match boron–proton cleanliness:
...
| **Princeton Fusion Systems (PFRC)** | D–$^3$He microreactor / propulsion | **No** — same D–D / residual-T neutronics | Already in §5.7 / Table 1; rapid T extraction stressed to limit 14 MeV neutrons [54–57]. |
...
**Plain-language lead-in.** Here the catalog goes company-by-company. Each write-up uses the same diligence checklist (§1.6). Executives can read the opening bullets and the **T** (technology-to-market) line; physicists should expect nuclear-data, kinetics, and materials detail comparable to a national-program overview [12].

Several key players are actively developing $p\text{-}^{11}\text{B}$ systems geared toward $Q > 1$ commercial electricity generation. Subsections below answer the §1.6 gates in fixed order (**F**, **K**, **R**, **A**, **L**, **C**, **M**, **T**; **B** only if relevant). Comparative scores appear in Table 1. Sister aneutronic fuels (D–$^3$He and others) are surveyed in §4.1.
...
*   **F — Fuel & nuclear:** Targets $p\text{-}^{11}\text{B}$ as preferred end-state fuel; cites modern cross sections $\sim 20\%$ above the data Rider used [5].
*   **K — Kinetics / Rider:** Injects high-energy neutral beams (NBI) tangentially into the FRC to create a non-Maxwellian, high-energy proton “tail,” raising fusion rate without a matching rise in bulk $T_e$ [5].
...
*   **M — Materials & capture:** Dry wall cooling (embedded He/water channels). **Inverse Cyclotron Converter (ICC)** at axial ends: escaping alphas helix in a tapering $B$ and drive $\sim 5$ MHz AC on segmented electrodes. Unobtainium: ICC electrode survival under continuous alpha bombardment; NBI grid longevity.
*   **T — T2M:** Multi-generation devices (Norman/C-2W → Norm); 2025 NBI-only formation on shortened Norm [36]; company claims Copernicus skip toward Da Vinci [37]; large private capital and patent portfolio (§9) [58,59].
*   **Experimental milestones:** With NIFS, first $p\text{-}^{11}\text{B}$ alphas in a magnetically confined plasma (LHD) [5,15,35].

### 5.2 HB11 Energy (Inertial Confinement - Laser Block Ignition)
HB11 Energy is pursuing a non-thermal, laser-driven "Proton Fast Ignition" / plasma-block scheme rooted in Hora’s ultrashort-pulse work [2,60].

*   **F — Fuel & nuclear:** $p\text{-}^{11}\text{B}$ pellets / blocks; Hora-lineage reactor IP [60].
*   **K — Kinetics / Rider:** Picosecond CPA lasers; ponderomotive block acceleration is argued to produce directed beams faster than ion–electron thermalization, suppressing thermal Bremsstrahlung [2]. A second, more contested pillar is an alpha-driven **avalanche**: fusion-born $^4\mathrm{He}$ elastically kicks protons into the reactivity peak, multiplying yield beyond single-pass beam-target rates [2,82].
*   **R — Radiation:** Nonthermal burn is meant to minimize *steady* Bremsstrahlung; a transient X-ray flash still loads carbon-composite / WC armor against cyclic shock. If the burn thermalizes, Rider-type $P_{Br}\gtrsim P_f$ returns in force (§2) [1,81].
...
*   **L — Lawson / $Q$:** Gain sought via nonthermal burn propagation (kinetic/rad-hydro models with Rochester TriForce) [7]; plant $Q$ still contingent on driver efficiency, rep-rate, and whether avalanche multiplication is real at reactor scale (Xie’s ICF zeroth-order caveat [40]).
*   **C — Confinement:** Laser ICF / block ignition (pulsed), often with a transient ultrahigh-$B$ coil for cylindrical trapping in reactor sketches [2].
*   **M — Materials & capture:** Double-walled He-cooled spherical chamber at 10–20 Hz. Megavolt spherical electrostatic grid decelerates $\sim 2.9$ MeV alphas. Open engineering risks: vacuum insulation under ionizing debris; final-optics survival at high rep-rate; and whether a spherical electrostatic collector can handle space-charge limits, electron shorting, and the *non*-monoenergetic alpha spectrum without excessive electrode heating [81].
*   **T — T2M:** Australian commercial program; PROBONO/Catania ecosystem participation [41,42].

**Moderated critique (community + literature).** Public discussion—including a 2021 r/fusion thread asking whether HB11 is a “superior” mechanism [81]—recapitulates points that also appear in the peer-reviewed record. (i) *Thermalization vs rare fusion:* Coulomb scattering dominates fusion, so directed beams tend to relax toward Maxwellians (and electron heating) before many fusions occur; nonthermal advantage is real only while that race is won [1,81]. (ii) *Avalanche kinematics:* treating alphas as monoenergetic $\sim 2.9$ MeV head-on proton kickers overstates the fraction of recoils that land in the cross-section peak; birth-energy breadth and impact-parameter spread broaden the proton distribution [81]. Shmatov’s Comment on the Eliezer–Hora avalanche paper argues that $\alpha$–$p$ elastic collisions could not produce the claimed physically important avalanche in the PALS-class experiments [82,83]; the authors published a Response [84], and the debate remains live rather than settled marketing. (iii) *Electrons:* fast ions preferentially couple to electrons; any avalanche or block-ignition power balance that omits that channel is incomplete [81]. (iv) *Direct conversion:* spherical electrostatic collection of charged alphas is attractive on paper but faces the space-charge / shorting / spectrum issues noted under **M** [81]. None of this voids laser $p\text{-}^{11}\text{B}$ as an experimental platform—Catania/PROBONO yields and HB11’s modeling program remain relevant (§3.7, §5.5)—but it cautions against scoring block ignition + avalanche + electrostatic DEC as already superior to other full-reactor paths in Table 1.
...
*   **M — Materials & capture:** Compact ST heat/Bremsstrahlung loads on central column and divertor (§9). Hybrid laser proton source (large numbers of 300–1000 keV protons) under study for magnetic devices [12].
...
*   **T — T2M / S:** $\sim 29$ papers, specialized codes, four 2025 U.S. applications [33]; Princeton approvals to incorporate as of July 2025; mid-2026 still no live site / no Series-A-scale raise (§5.7)—strong **F**–**A** and **S** (in-silico-first) paper record, weak **T**/**H**.

### 5.7 Princeton / PPPL Spinouts: Commercial Status, Filings, and Prognosis
Princeton University and PPPL do not commercialize through a single exclusive vehicle. Distinct inventor groups license distinct IP into parallel companies—closer to a university portfolio than to a Symbolics-versus-Lisp-Machine civil war over one product line. On the §1.6 rubric, **Thea** and **Princeton Fusion Systems** are sister spinouts that close gate **T** (and, for Thea, federal **L**-path design review) on *non*-$p\text{-}^{11}\text{B}$ fuels; **Pale Blue** is the house $p\text{-}^{11}\text{B}$ bet still early on **T**. (Princeton NuEnergy, a plasma-based battery-materials spinout, is omitted here as outside electricity-from-fusion scope.)

#### 5.7.1 Thea Energy (formerly Princeton Stellarators)
...
#### 5.7.2 Pale Blue Fusion (Fisch CHARM / ARPA-E OPEN 2021)
*   **Technology:** CHARM centrifugal multi-chamber open-field concept for economical $p\text{-}^{11}\text{B}$ (§5.6) [33]—strong paper record on gates **K**/**A**/**R**, pending hardware on **L**/**M**/**T**.
...
#### 5.7.3 Princeton Fusion Systems (PFRC / Princeton Satellite Systems)
*   **Technology:** Princeton Field-Reversed Configuration (**PFRC**) with odd-parity rotating magnetic field (RMF$_o$) heating, invented by Samuel Cohen at PPPL; commercialized via **Princeton Fusion Systems** (subsidiary/affiliate of Princeton Satellite Systems) for compact 1–10 MW-class microreactors and space propulsion (Direct Fusion Drive / “Starfire”), targeting advanced fuels (**D–$^3$He**) rather than $p\text{-}^{11}\text{B}$—so gates **F**/**B** differ from this survey’s core fuel [54,55].
...
### 5.8 Compact MEC / IEC: Avalanche Energy (Orbitron) and JIHT Kurilenkov NVD
...
| **TAE** [5,35–37] | FRC | ● | ● | ● | ◐ | ◐ | ◐ | ● | ◐ | ● | LHD alphas; Norm NBI-only; ICC/**M** and plant **L** still open. |
| **LHD / NIFS** [15] | Helical | ● | ◐ | — | — | ○ | — | ● | ◐ | ● | Facility demo of magnetic $p\text{-}^{11}\text{B}$ alphas; not a plant design. |
| **HB11** [2,7,81–84] | Laser ICF | ● | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | Block ignition real R&D; avalanche + spherical DEC contested (§5.2). |
...
| **Thea Energy** [46–50] | Planar stellarator | — | — | — | — | ● | ◐ | ● | ● | ◐ | D–T path; DOE Helios design certified; digital-twin / Omniverse ecosystem (§8). |
| **PFS / PFRC** [54–57] | PFRC | — | — | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ● | D–$^3$He microreactor; ARPA-E/INFUSE; neutron-*poor*, not $p\text{-}^{11}\text{B}$-clean (§4.1). |
| **Helion Energy** [77,78] | Pulsed FRC | — | — | ◐ | ◐ | ◐ | ◐ | ● | ◐ | ● | D–$^3$He + in-house $^3\mathrm{He}$ breeding; largest capitalized sister-fuel path; **not** fully aneutronic (§4.1). |
| **Avalanche Energy** [66,67,87] | Orbitron (MEC) | ◐ | ◐ | ◐ | ◐ | ◐ | ◐ | ● | ● | ● | Compact magneto-electrostatic; D–T / neutron-source near term; **$p\text{-}^{11}\text{B}$-capable** per design claim; FusionWERX + high **H**/**S** (§4, §8). |
| **JIHT / Kurilenkov NVD** [85,86,88] | NVD–IEC (VC) | ● | ● | ◐ | ◐ | ○ | ◐ | ○ | ● | ● | Russian lab miniature vacuum discharge; experimental $p\text{-}^{11}\text{B}$ alphas + KARAT scaling; sits with Avalanche in compact electrostatic family (§4). |

Gate letters: **C** confinement class (named in cell); **F** fuel/nuclear; **K** kinetics/Rider; **R** radiation; **A** ash/impurities; **L** Lawson/engineering $Q$; **M** materials & energy capture; **T** technology-to-market; **S** in-silico / digital-twin iteration; **H** hardware iteration (§1.6, §8).

---

## 8. In Silico Design Software and Digital Twins (Gates S / H)
**Plain-language lead-in.** Who designs on computers how fast (**S**), versus who builds and shoots hardware how often (**H**). Table 3 is a shopping list for tools and how to obtain them—open stacks vs closed SaaS.

In computational medicine and biology, testing a therapy or organ model on a computer is usually called **in silico**. Fusion’s marketing synonym is **digital twin**: a virtual plant that couples geometry, multiphysics, cost, and—eventually—live telemetry [64,65]. Both labels answer gate **S**. Gate **H** is the complementary question: how fast can you cut metal and shoot plasma? A healthy program needs both; **S** without V&V against **H** can manufacture PowerPoint reactors, while **H** without **S** can thrash hardware.

Two poles illustrate the trade. **Pale Blue / CHARM** and related theory houses explicitly prioritize an *in-silico power-positive* reactor before major hardware [33]—high **S**, low **H** so far. **Avalanche Energy** is the contrasting commercial bet: desk-scale **Orbitron** magneto-electrostatic machines, rapid design–build–test–learn, FusionWERX as industry test infrastructure, peer-reviewed Orbitron confinement papers, and open PIC/neutronics stacks (WarpX, OpenMC) on multi-cloud HPC [66,67,87]—high **H**, serious **S**, and a staged fuel path (D–T / neutrons first; $p\text{-}^{11}\text{B}$ claimed as a lighter, low-shielding option for later packs). Alongside it in the compact electrostatic / IEC neighborhood sits the **JIHT RAS Kurilenkov NVD** line: reverse-polarity virtual-cathode discharges with published $p\text{-}^{11}\text{B}$ α detection and KARAT PIC geometry scaling [85,86,88]—academic rather than venture-backed, but already on $p\text{-}^{11}\text{B}$ fuel and not a fusor write-off. Neither Avalanche nor NVD alone closes gates **L** and **M**, but both are full reactor *architectures* in this survey’s gate-**C** sense.

### 8.1 Open vs closed source
...
*   **Closed / SaaS / enterprise:** Faster packaging and support; opaque physics assumptions; access by contract, NDAs, or seat licenses. Fine for EPCs and investors; weak for peer review of **L**.
...
### 8.2 Catalog: claims and how to get access

### Table 3: Selected fusion design / digital-twin software (access snapshot, mid-2026)

| Tool / stack | Claim | License / openness | How to get access |
| :--- | :--- | :--- | :--- |
| **OpenMC** | Monte Carlo neutron/photon transport; CAD→DAGMC workflows common in FPP studies | Open source (MIT) | `conda install -c conda-forge openmc`, Docker Hub images, or build from https://github.com/openmc-dev/openmc ; docs: https://docs.openmc.org [69] |
...
| **WarpX / pyWarpX** | Exascale PIC; used in industrial rapid-iteration loops (e.g., Avalanche) | Open (BSD-style; DOE ECP lineage) | https://github.com/ECP-WarpX/WarpX ; Spack/containers common [67] |
...
| **Fisch/CHARM stack** ((PB)$^2$, S$^5$, R$^3$FP, MITNS) | Specialized nonthermal / multi-chamber / radiative FP tools for Pale Blue path | Academic / not productized | Described in ARPA-E materials [33]; no public SaaS—access via collaboration with Princeton authors |
| **FREDA** (ORNL SciDAC) | Whole-facility plasma+engineering digital-twin *framework*; multi-fidelity optimization; IPS backbone; aims mostly open | Federal R&D (2023–27); not a pip-install product | Via DOE/ORNL SciDAC collaboration; overview: ORNL news and IAEA/APS materials [68,72] |
...
| **VeloAlpha FusionAlpha** | AI simulator to test reactor designs before hardware (EDA analogy); founded 2026 by H. Xie | Closed startup product (seed stage) | No public self-serve download as of mid-2026 press; company/press channels [75,76] |
| **MCNP** | Gold-standard Monte Carlo (still widely required) | Export-controlled / licensed (LANL) | RSICC / institutional licence—not freely redistributable |
...
## 9. Materials Science and Cooling Challenges (Gate M)
...
### 9.1 Cyclic Thermal Shock and Spallation
...
### 9.2 Impurity and Ash Poisoning
...
**Plain-language lead-in.** Who owns what on paper—companies, trademarks, and patents. Useful for diligence; not a substitute for physics scores in Table 1.
...
| **Helion Energy** | Live (`helionenergy.com`) | WA company | Active brand | Largest capitalized *sister-fuel* (D–$^3$He) IP bench; §4.1. |
...
### Table 2c: What the citation graph shows (cross-firm prior art)
...
40. **Xie, H.** (2023). *Introduction to Fusion Ignition Principles: Zeroth Order Factors of Fusion Energy Research*. USTC Press, Hefei. English translation PDF (primarily machine-translated; dated 24 October 2024) at https://github.com/hsxie/fusionbook/blob/main/enfusionbook.pdf; also arXiv:2410.18054. The Ch. 8 scheme-evaluation checklist is adapted in §1.6 of this survey for $p\text{-}^{11}\text{B}$ electricity paths.
...
77. **Wikipedia contributors.** *Aneutronic fusion*. Wikimedia Foundation. https://en.wikipedia.org/wiki/Aneutronic_fusion (accessed mid-2026). Used in §4.1 for the statutory $\lesssim 1\%$ neutron-energy definition, candidate reaction table, residual $p\text{-}^{11}\text{B}$ radiation, and the D–$^3$He / D–D side-reaction caveat.
